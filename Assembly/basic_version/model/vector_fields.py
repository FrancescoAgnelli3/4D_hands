...
class VectorField_g(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers, num_nodes, cheb_k, embed_dim,
        ...
        if self.g_type == 'agc':
            ...
            self.cheb_k = cheb_k
            self.weights_pool = nn.Parameter(torch.randn(embed_dim, cheb_k, hidden_hidden_channels, hidden_hidden_channels))
            self.bias_pool = nn.Parameter(torch.randn(embed_dim, hidden_hidden_channels))
