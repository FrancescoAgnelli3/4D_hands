...
def train(model, train_data, val_data, params, train_writer, val_writer):
    ...
    working_dir = params['working_dir'] + "/" + params['graph_type']
    if os.path.exists(working_dir) == False:
        os.makedirs(working_dir)
    best_path = os.path.join(working_dir, 'best_model.pt')
    checkpoint_dir = os.path.join(working_dir, 'model_checkpoint.pt')
    training_path = os.path.join(working_dir, 'training_checkpoint.pt')
    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        for batch_ind, batch in enumerate(train_data_loader):
            ...
            loss, loss_nll, loss_kl, edges_train = model.calculate_loss(inputs, is_train=True, return_edges=True)
            loss.backward()
            if verbose:
        ...
        with torch.no_grad():
            for batch_ind, batch in enumerate(val_data_loader):
                ...
                loss, loss_nll, loss_kl, edges_val = model.calculate_loss(inputs, is_train=False, teacher_forcing=val_teacher_forcing, return_edges=True)
