...
def eval_forward_prediction(model, dataset, burn_in_steps, forward_pred_steps, params, return_total_errors=False):
    ...
    gpu = False # params.get('gpu', False)
    batch_size = 16 # params.get('batch_size', 1000)
    ...
    for batch_ind, batch in enumerate(data_loader):
        inputs = batch['inputs']
        #print(inputs.shape) # torch.Size([16, 49, 31, 6])
        with torch.no_grad():
            model_inputs = inputs[:, :burn_in_steps] # torch.Size([16, 39, 31, 6])
            gt_predictions = inputs[:, burn_in_steps:burn_in_steps+forward_pred_steps]
            GTs.append(gt_predictions)
            #print(gt_predictions.shape) #torch.Size([16, 10, 31, 6])
            if gpu:
                model_inputs = model_inputs.cuda(non_blocking=True)
            model_preds = model.predict_future(model_inputs, forward_pred_steps).cpu() # torch.Size([16, 10, 31, 6])
    ...
    if return_total_errors:
        return torch.cat(all_errors, dim=0), predictions, GTs
    else:
        return total_se / len(dataset), predictions, GTs
