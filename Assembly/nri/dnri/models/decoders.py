...
class GraphRNNDecoder(nn.Module):
    def __init__(self, params):
        ...
        if params['graph_type'] == 'semantic':
            ...
            edges = np.ones(42) - np.eye(42)
            send_edges = np.where(edges)[0]
            recv_edges = np.where(edges)[1]

            a_base = torch.from_numpy(np.concatenate([send_edges[:, None], recv_edges[:, None]], axis=1))
            self.A_new = torch.zeros((42*41))
            num = 0
            for i,j in a_base:
                for k,l in A:
                    if i == k and j == l:
    ...
    def single_step_forward(self, inputs, rel_type, hidden):
        ...
        if self.graph_type == 'semantic':
            rel_type = torch.cat([rel_type, self.A_new.to("cuda").unsqueeze(0).expand(rel_type.shape[0],self.A_new.shape[-1]).unsqueeze(-1)], dim=-1)
